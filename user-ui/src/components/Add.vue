<template>
  <div class="container " style=" width: 800px; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);border-radius: 4px">
    <h2 class="page-header" style="text-align: center;margin-top: 30px;">添加用户</h2>
    <router-link to="/"> 
            <el-button style="float:right;" type="primary" plain>返回</el-button>
        </router-link>
        ...
        this.$message({
          ...
          if(newAdds){
            this.$confirm('是否添加该用户', '提示', {
                ...
                type:'warning'})
                .then(()=>{
                    axios.post("http://localhost:3000/users",newAdds)
                    .then((response)=>{
